...
class ArxivPDFProcessor:
    ...
    def download_paper_pdf(self, paper: Dict) -> Tuple[bool, str]:
        ...
        while current_retry < Config.MAX_DOWNLOAD_RETRIES:
            try:
                # 每次重试使用不同的User-Agent
                headers = {'User-Agent': self.user_agents[current_retry % len(self.user_agents)]}
                
                logger.info(f"正在下载论文PDF: {paper['id']} (尝试 {current_retry + 1}/{Config.MAX_DOWNLOAD_RETRIES})")
                ...
                logger.warning(f"下载PDF超时 (尝试 {current_retry + 1}/{Config.MAX_DOWNLOAD_RETRIES})")
                current_retry += 1
                time.sleep(Config.RETRY_DELAY)
                continue
                
            except requests.exceptions.RequestException as e:
                logger.warning(f"下载PDF时网络错误 (尝试 {current_retry + 1}/{Config.MAX_DOWNLOAD_RETRIES}): {e}")
                current_retry += 1
                time.sleep(Config.RETRY_DELAY)
                continue
        ...
        # 所有重试都失败
        error_msg = f"下载PDF失败，已重试 {Config.MAX_DOWNLOAD_RETRIES} 次"
